...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(x => google.exists(term => x.text.contains(term)))
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(x => apple.exists(term => x.text.contains(term)))
  }
